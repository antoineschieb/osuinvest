...
async def buy_stock(buyer_name: str, stock_name, quantity: float):
    ...
    if quantity > 0:
        ...
        if transaction_price > buyer.cash_balance:
            return f'{buyer_name} does not have enough cash (${buyer.cash_balance}) to perform this transaction (${transaction_price}).'
        
        # check if enough shares are available for sale
        if stock.total_shares - stock.sold_shares < quantity:
            return f'The total number of available shares {stock.total_shares - stock.sold_shares} is not sufficient to perform this transaction.'
    elif quantity < 0:
        ...
        if portfolio.loc[stock_name,'shares_owned'] < abs(quantity):
            return f'{buyer_name} does not have enough {id_name[stock_name]} shares ({portfolio.loc[stock_name,"shares_owned"]}) to perform this transaction.'
        ...
        return f'Quantity traded can not be zero.'
